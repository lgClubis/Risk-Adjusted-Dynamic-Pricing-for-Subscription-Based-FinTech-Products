...
def simulate_price_grid(
    ...
    rows = []
    for p in price_grid:
        hazards = predict_hazards(model, feature_columns, base_profile, price=float(p), horizon_months=horizon_months)
        S = survival_from_hazards(hazards)
        ...
        margin = (1.0 - variable_cost_rate) * float(p) - float(fixed_cost)
